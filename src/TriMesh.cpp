...
void TriMesh::storeFacesPoints2() {
	// 计算法向量
	if (vertex_normals.size() == 0)
		computeVertexNormals();
...
void TriMesh::generateSphereTexture()
{
	vertex_textures.clear();
	for (int i = 0; i < vertex_positions.size(); i++) {
		glm::vec3 pos = glm::normalize(vertex_positions[i]);
...
void TriMesh::readOff2(const std::string& filename)
{
    // fin打开文件读取文件信息
    if (filename.empty())
    {
...
void TriMesh::readObj(const std::string& filename) {
	...
	while (std::getline(fin, line)) {
		...
		type = line.substr(0, first_space); // 读取第一个空格前的子串，作为该行类型
		...
		if (type == "f") {
			...
			sscanf(line.c_str() + pos, "%d%c%d%c%d", &a0, &slash, &b0, &slash, &c0);
			pos = line.find_first_of(' ', pos) + 1;
			sscanf(line.c_str() + pos, "%d%c%d%c%d", &a1, &slash, &b1, &slash, &c1);
			pos = line.find_first_of(' ', pos) + 1;
			sscanf(line.c_str() + pos, "%d%c%d%c%d", &a2, &slash, &b2, &slash, &c2);

			// 顶点索引从1开始，但是向量索引从0开始，因此需要-1
		...
		// 如果该行以“v”开头，说明是点坐标信息
			...
			sscanf(line.c_str() + 2, "%f %f %f", &_x, &_y, &_z);
		...
		// 法向量
			...
			sscanf(line.c_str() + 3, "%f %f %f", &_x, &_y, &_z);
		...
		// 纹理坐标
			...
			sscanf(line.c_str() + 3, "%f %f", &_x, &_y);
			vertex_textures.push_back(glm::vec2(_x, _y));
			continue;
		}
	}

	// 由于存在纹理，颜色信息可以随意指定（此处使用法向量）
	...
	std::cout << "Function: readObj used: " << time_used << " seconds" << std::endl;
}
...
glm::mat4 Light::getShadowProjectionMatrix2()
{
	float lx, ly, lz;
