...
void MeshPainter::bindObjectAndData2(TriMesh* mesh, openGLObject& object, const std::string &vshader, const std::string &fshader2, const std::string &texture_image) {
	...
	if (!textures.empty()) {
		bufferSize += textures.size() * sizeof(glm::vec2);
	}
	glGenBuffers(1, &object.vbo);
	glBindBuffer(GL_ARRAY_BUFFER, object.vbo);
	...
	if (!textures.empty()) {
		glBufferSubData(GL_ARRAY_BUFFER, (mesh->getPoints().size() + mesh->getColors().size() + mesh->getNormals().size()) * sizeof(glm::vec3), 
		               textures.size() * sizeof(glm::vec2), &textures[0]);
	}

	object.vshader = vshader;
	object.fshader2 = fshader2;
	object.program = InitShader(object.vshader.c_str(), object.fshader2.c_str());
	...
	if (!textures.empty()) {
		object.tLocation = glGetAttribLocation(object.program, "vTexture");
		if (object.tLocation != -1) {
			glEnableVertexAttribArray(object.tLocation);
			glVertexAttribPointer(object.tLocation, 2, GL_FLOAT, GL_FALSE, 0, 
			                       BUFFER_OFFSET((mesh->getPoints().size() + mesh->getColors().size() + mesh->getNormals().size()) * sizeof(glm::vec3)));
		}
	}
...
void MeshPainter::addMesh2( TriMesh* mesh, const std::string &name, const std::string &vshader, const std::string &fshader2, const std::string &texture_image ){
	mesh_names.push_back(name);
    meshes.push_back(mesh);

    openGLObject object;
    // 绑定openGL对象，并传递顶点属性的数据
    bindObjectAndData2(mesh, object, vshader, fshader2, texture_image);
...
void MeshPainter::drawMesh(int i, glm::mat4 modelMatrix, Light *light, Camera* camera, int isShadow){
	...
	glUniformMatrix4fv(object.viewLocation, 1, GL_FALSE, &camera->viewMatrix[0][0]);
	glUniformMatrix4fv(object.projectionLocation, 1, GL_FALSE, &camera->projMatrix[0][0]);
	// 如果着色器 isShadow 设置为0，表示正常绘制的颜色，如果是1着表示阴影
	glUniform1i(object.shadowLocation, isShadow);
...
void MeshPainter::drawMesh2(int i, glm::mat4 modelMatrix, Light *light, Camera* camera, int isShadow){
	...
	// 如果对象有纹理，绑定纹理
	GLint useTextureLocation = glGetUniformLocation(object.program, "useTexture");
	if (!object.texture_image.empty() && object.texture != 0) {
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, object.texture);
		GLint textureLocation = glGetUniformLocation(object.program, "texture");
...
void MeshPainter::updateTexture(int index, const std::string &texture_image) {
	if (index >= 0 && index < opengl_objects.size()) {
		openGLObject& obj = opengl_objects[index];
		if (obj.texture != 0) {
			glDeleteTextures(1, &obj.texture);
			obj.texture = 0;
		}
		obj.texture_image = texture_image;
		if (!texture_image.empty()) {
			glGenTextures(1, &obj.texture);
			load_texture_STBImage(texture_image, obj.texture);
		}
	}
}